...
class ProxyFrameworkPool:
    ...
    def prepare(
        ...
        *,
        ...
        self._progress(progress_callback, SearchEventName.PROXY_PREPARE_STARTED, "Baixando lista de proxies...", {"provider": self.provider_name, "sources": len(sources)})
        ...
        try:
            self._progress(progress_callback, SearchEventName.PROXY_TESTING, "Testando proxies...", {"threads": threads, "timeout": timeout})
            ...
            if not working:
                ...
                self._progress(progress_callback, SearchEventName.PROXY_NO_WORKING, "Nenhuma proxy funcional encontrada!", {"total": len(results)})
                raise RuntimeError("Nenhuma proxy funcional encontrada para o portal.")

            self._progress(progress_callback, SearchEventName.PROXY_TESTED, f"{len(working)} proxies OK, iniciando bridges...", {"working": len(working), "total": len(results)})
            active = self._proxy.start(amounts=valid_count, auto_test=False)

            self._progress(progress_callback, SearchEventName.PROXY_VERIFYING, f"Verificando {len(active)} bridges...", {"expected": len(active)})
            ...
            with ThreadPoolExecutor(max_workers=min(len(active), 10)) as executor:
                ...
                for future in as_completed(fut_map):
                    ...
                    if result is not None:
                        self._progress(progress_callback, SearchEventName.PROXY_BRIDGE_VERIFIED, f"Bridge {i+1}/{len(active)} OK", {"index": i, "total": len(active)})
                    else:
                        self._progress(progress_callback, SearchEventName.PROXY_BRIDGE_FAILED, f"Bridge {i+1}/{len(active)} falhou", {"index": i, "total": len(active)})
            ...
            if not verified_bridges:
                ...
                self._progress(progress_callback, SearchEventName.PROXY_NO_BRIDGES, "Nenhuma bridge ativa apos verificacao!", {})
                raise RuntimeError(
            ...
            self._progress(progress_callback, SearchEventName.PROXY_PREPARE_FINISHED, f"{len(bridges)} bridges ativas", {"bridges": len(bridges)})
            return bridges
        except Exception:
            self._proxy.stop()
            raise
    ...
    @staticmethod
    def _progress(callback: Callable[[str, str, dict], None] | None, name: str | SearchEventName, message: str, payload: dict) -> None:
        if callback:
            try:
                event_name = name.value if isinstance(name, SearchEventName) else name
                callback(event_name, message, payload)
            except Exception:
                pass
